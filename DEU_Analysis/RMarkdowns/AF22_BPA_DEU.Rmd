...
```{r}
source("../../lvfm_core/R/lvfm_core.R")
```

The legacy factor mapping is now provided by `lvfm_core`, which keeps the
same `BPA_F()` entry point while avoiding a duplicated inlined definition.
